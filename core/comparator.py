...
# --- Always append final "Straight Comparison" sheet (all originals) ---
try:
    all_sheets = discover_scenario_sheets(src_workbook, log_func=log_func)
except Exception:
    # Fallback: include everything if detection fails
    all_sheets = list_sheets(src_workbook)

straight_name = "Straight Comparison"
if straight_name in wb.sheetnames:
    k = 2
    while f"{straight_name} ({k})" in wb.sheetnames:
        k += 1
    straight_name = f"{straight_name} ({k})"

write_straight_comparison_sheet(
    wb,
    straight_name,
    src_workbook,
    threshold=threshold,
    sheet_names=all_sheets,  # IMPORTANT: all originals, not just selected comparisons
    log_func=log_func,
