...
def build_pair_comparison_df(
    ...
    if not df_all.empty:
        ...
        df_all = df_all.sort_values(by=["CaseType", "_SortKey"], ascending=[True, False], na_position="last").drop(columns=["_SortKey"])
    return df_all
...
def build_batch_comparison_workbook(
    ...
    **kwargs,
    ...
    # NEW: final Straight Comparison
    try:
        originals = _ordered_original_sheets(src_workbook, pairs)
        df_straight, case_labels = build_straight_comparison_df(src_workbook, originals, threshold=threshold, log_func=log_func)
