...
def _default_workbook_path(root_folder: str) -> str:
    return os.path.join(root_folder, "Combined_ViolationCTG_Comparison.xlsx")


def _build_simple_workbook(root_folder, folder_to_case_csvs, log_func=None, workbook_path=None):
    """
    ...
    if not workbook_path:
        workbook_path = _default_workbook_path(root_folder)
...
def build_workbook(root_folder, folder_to_case_csvs, log_func=None, workbook_path=None):
    """
    ...
    workbook_path:
        If provided, this exact output path will be used.
        Otherwise defaults to: root_folder/Combined_ViolationCTG_Comparison.xlsx
    """
    if not folder_to_case_csvs:
        if log_func:
            log_func("No data to build combined workbook.")
    ...
    if not OPENPYXL_AVAILABLE:
        ...
        return _build_simple_workbook(root_folder, folder_to_case_csvs, log_func, workbook_path)
    ...
    for folder_name, df in scenario_data.items():
        ...
        # Shift everything down by 1 row (leave row 1 blank for nicer spacing)
        current_row = 2

        # Process blocks in fixed order
        for label in TARGET_PATTERNS:
            block_df = df[df["CaseType"] == label]
            if block_df.empty:
                continue

            # Sort by highest Percent Loading (LimViolPct) first (descending)
            # This fixes the "main resulting issues not ordered by percent" problem.
            if "LimViolPct" in block_df.columns:
                try:
                    block_df = block_df.sort_values(by="LimViolPct", ascending=False, kind="mergesort")
                except Exception:
                    pass
